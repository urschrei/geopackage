# Summary

[Introduction](introduction.md)

# Tutorial

- [Your first GeoPackage](tutorial/first-geopackage.md)

# How-to guides

- [How to add a spatial index to an existing layer](how-to/add-spatial-index.md)
- [How to repair a file's spatial indexes](how-to/repair-spatial-indexes.md)
- [How to copy features between files without decoding geometry](how-to/copy-features.md)
- [How to read a layer as Arrow record batches](how-to/read-arrow.md)
- [How to validate a GeoPackage and act on the findings](how-to/validate.md)

# Explanation

- [The layer extent, and why it cannot be trusted](explanation/extent.md)
- [Transactions, and who commits](explanation/transactions.md)
- [The spatial index: structure, contents, and the bulk build](explanation/spatial-index.md)
- [Geometry storage: GPB, WKB, and curve types](explanation/geometry.md)
- [Tiled gridded coverages, and why they are not tile pyramids](explanation/coverages.md)

# Reference

- [Reference](reference.md)
