...
//! "how many allocations". The build is dominated by SQLite rather than by CPU
//! in Rust, so allocation count and byte volume are what changes in this path
//! show up as, and a wall-clock bench mostly measures the database.
//!
...
//! # Running it
//!
//! On Linux, with Valgrind installed and the runner that matches the pinned
//! library version:
//!
//! ```text
//! cargo install --version 0.19.2 gungraun-runner
//! cargo bench -p geopackage --bench alloc
//! ```
//!
//! The runner is a separate binary that has to be on `PATH` (or named by
//! `GUNGRAUN_RUNNER`); the bench exits with an error naming it if it is absent.
//! Its version has to match the `gungraun` dev-dependency, which is why that
//! dependency is pinned exactly.
//!
//! Valgrind has no macOS arm64 port: Homebrew's formula is `depends_on :linux`
//! and ships Linux bottles only. The bench still compiles on macOS, so
//! `cargo bench --no-run` and ordinary `cargo check` stay useful there, but
//! producing numbers needs a Linux machine or container.
