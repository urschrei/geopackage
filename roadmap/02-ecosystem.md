...
appear in a GeoPackage, so "should we support GeoTIFF?" is really three
...
`ModelPixelScale` and no `ModelTiepoint`. What it asks for, reading the
[migrated spec source](https://github.com/opengeospatial/geopackage/tree/master/spec/2d-gridded-coverage),
is a deliberately tiny profile of baseline TIFF:
...
small gap it looks like, because `tiff` already carries the tag constants
...
- The encoder is **strip-only**; there is no `TileWidth`/`TileLength` write
  ...
  required behaviour.
- Encoder predictors: `Horizontal` only, and it is refused for `IEEEFP`;
...
1. **Coverage payload validation: no dependency.** Requirements 16 to 20 are
   ...
   conformance check can actually assert. Planned as phase 1 of a coverage
   milestone.
