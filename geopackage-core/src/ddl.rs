...
/// the two tables of the tiled gridded coverage extension. Created with the
/// first coverage.
///
/// The SQL is verbatim from the spec source, including the single-quoted table
/// and column names. SQLite reads a single-quoted name in these positions as an
/// identifier. GDAL writes a variant in which the `CHECK` is part of the
/// foreign key constraint clause. Both parse, and this crate does not read the
/// DDL back.
...
/// `gpkg_2d_gridded_tile_ancillary` (OGC 17-066r2 Annex C), the per-tile
/// table. Created with the first coverage.
///
/// The table has no `ON DELETE CASCADE` on `tpudt_id`. A delete of a tile does
/// not delete its ancillary row, so a writer must delete both.
