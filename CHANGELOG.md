...
- **Tiled gridded coverages** (OGC 17-066r2, `gpkg_2d_gridded_coverage`):
  ...
  still refuses one. `CoverageBuilder` declares what the samples mean;
  `CoverageWriter` keeps the tile and its `gpkg_2d_gridded_tile_ancillary` row
  in step, in both directions, since the normative table definition has no
...
- `geopackage_core::coverage`: the extension's payload profile, checked from
  ...
  `CoverageDatatype::check_payload` is the rule tying a payload to the
  `datatype` its coverage declares (Requirements 13 and 14). No sample is
  decoded anywhere, so Requirement 21 — every pixel valid, no NaN, no Inf —
  is out of reach and documented as such.
- `GeoPackage::validate` checks coverages: the ancillary rows (Requirements 1,
  ...
  the coverage's own `datatype`. Six `Finding` variants carry the results; a
...
- `TileMatrixSet::tile_at` returns `None` for a tile matrix declaring no
  ...
  grid and `validate` refuses it, but `TileMatrix` is constructible directly
  and one read from a file written elsewhere has been validated by nobody, so
...
- **[The book](https://urschrei.github.io/geopackage/)**, an mdBook guide
  structured on Diataxis lines: a tutorial, how-to guides for common tasks,
  and the design reasoning behind the library's choices. Its Rust code blocks
  are compiled against the workspace in CI, so the examples cannot drift from
  the API. The API reference remains on docs.rs.
...
- Two divergences of Apple's hardened system SQLite are absorbed, so the
  ...
  `-wal`/`-shm` sidecars that persist-WAL leaves behind are removed after the
  close path's journal reset.
- CI covers both linkings: the existing three-OS matrix builds with
  `--all-features` (which now includes `bundled`), and a new Linux and macOS
  leg tests the system-linked default.
