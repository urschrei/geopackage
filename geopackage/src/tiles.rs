...
/// What creating a tile table needs to know, whichever kind of content it
/// holds.
///
/// A tile pyramid and a tiled gridded coverage share their whole physical
/// shape -- the user table, the matrix set, the zoom levels, the
/// `gpkg_contents` row -- and differ in the `data_type` that row declares and
/// in what else goes beside it. This is the shared part, so
/// [`crate::Coverage`] does not reimplement it.
...
impl GeoPackage {
    ...
    /// Checks everything that can be checked before anything is written,
    /// returning whether the pyramid needs `gpkg_zoom_other`.
    ///
    /// Separate from [`Self::write_pyramid`] so a refusal happens outside the
    /// transaction rather than inside one that then rolls back.
    ...
    /// Writes the user table and the catalogue rows, inside a transaction the
    /// caller owns.
    ...
    /// Opens the tile machinery over a `gpkg_contents` row of the given data
    /// type.
    ///
    /// The tile table, its matrix set and its zoom levels are shaped the same
    /// whether the payloads are pictures or measurements, so a coverage
    /// (`2d-gridded-coverage`, Requirement 5) borrows all of it. What it does
    /// not borrow is the public handle: [`crate::Coverage`] keeps this one
    /// private, so a coverage never reaches a writer that knows nothing about
    /// its ancillary rows.
