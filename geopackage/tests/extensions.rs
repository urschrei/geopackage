...
fn every_extension_in_the_committed_fixtures_is_classified() {
    ...
    assert_eq!(
        seen,
        vec![
            // The coverage fixture. Implemented since the write path landed:
            // this crate reads a coverage, checks its payloads and writes one.
            (
                "gpkg_2d_gridded_coverage".to_owned(),
                ExtensionSupport::Implemented
            ),
            // Also the coverage fixture: GDAL writes a WKT2 definition beside
            // the EPSG:3857 row it seeds.
