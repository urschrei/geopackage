...
/// `gpkg_2d_gridded_coverage_ancillary` (OGC 17-066r2 Annex C), the first of
/// the two tables the tiled gridded coverage extension defines. Created lazily
/// with the first coverage.
///
/// Verbatim from the spec source, single-quoted table and column names
/// included: SQLite reads a single-quoted identifier in these positions as an
/// identifier, and "do not improve normative text" applies to its quoting as
/// much as to its columns. GDAL writes a variant that folds the `CHECK` into
/// the foreign key's constraint clause; both parse, and nothing here reads the
/// DDL back.
pub const CREATE_GPKG_2D_GRIDDED_COVERAGE_ANCILLARY: &str = "\
...
/// `gpkg_2d_gridded_tile_ancillary` (OGC 17-066r2 Annex C), the per-tile half.
/// Created lazily with the first coverage.
///
/// Note what is *not* here: an `ON DELETE CASCADE` on `tpudt_id`. Deleting a
/// tile does not delete its ancillary row, so a writer has to delete both, and
/// [`crate::coverage::TILE_ANCILLARY_TABLE`] names the table that has to be
/// kept in step.
pub const CREATE_GPKG_2D_GRIDDED_TILE_ANCILLARY: &str = "\
