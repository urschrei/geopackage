...
//! - [`gdal_reads_a_coverage_we_wrote`] writes a tiled gridded coverage
//!   carrying a payload GDAL encoded, and has GDAL read the samples back out
//!   of it -- the one check this workspace cannot make itself, since it
//!   decodes no pixels.
...
/// The committed coverage fixture's float32 tile: a payload GDAL encoded,
/// rather than the bare headers the unit tests build.
...
fn gdal_reads_a_coverage_we_wrote() {
    ...
    let tile = fixture_coverage_tile();
    {
        let gpkg = GeoPackage::create(&path).unwrap();
        gpkg.add_epsg_srs(3857).unwrap();
        // The fixture's tile is 64 pixels square, so the grid is one tile of
        // 64 over a 64-unit extent: a pixel is a ground unit.
    ...
    // What the coverage says about its samples, read back by another
    // implementation: the type from the payload, the null from the ancillary
    // row we wrote.
    assert_eq!(band["type"], "Float32");
    assert_eq!(band["noDataValue"], serde_json::json!(-9999.0));

    // And the samples themselves, which this crate never decoded on either
    // side of the write.
